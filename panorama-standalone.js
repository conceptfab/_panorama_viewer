(function () {
  ...
  // --- DANE: panoramy, linki, infospoty (edycja tylko tutaj) ---
  // position = domyślny kierunek kamery po wejściu [x, y, z].
  // links = portale do innych panoram: to = indeks panoramy, position = [x, y, z] na TEJ panoramie.
  // Współrzędne do linków: Ctrl+klik w widoku → konsola "Kliknięcie w: x=..., y=..., z=...".
  var PANORAMA_CONFIG = [
    {
      file: '4K_pano_00.webp',
      position: [1000, 0, 0],
      links: [
        { to: 1, position: [7000, -2000, 0] },
        { to: 2, position: [5200, -500, -2000] },
      ],
    },
    {
      file: '4K_pano_01.webp',
      position: [3828, -356, 3183],
      links: [
        { to: 2, position: [4500, -52.29, -2044.24] },
        { to: 0, position: [3840.78, -94.24, 3185.35] },
      ],
    },
    {
      file: '4K_pano_02.webp',
      position: [85, 270, 4984],
      links: [
        { to: 0, position: [1310.1, -229.4, 4808.93] },
        { to: 1, position: [-4971.36, -353.86, 281.15] },
      ],
    },
  ];

  var INFOSPOTS_CONFIG = [
    {
      panoramaIndex: 0,
      position: [2000, -180, 40],
      hoverText: 'Kontakt',
      clickText: 'Kontakt – informacje kontaktowe.',
      visible: false,
    },
    {
      panoramaIndex: 0,
      ...
      hoverText: 'Kliknij.',
      clickText: 'To jest szczegółowa informacja w oknie!',
      visible: false,
    },
  ];
  ...
  function setProgress(percent) {
    if (splashProgressBar) {
      splashProgressBar.style.width = Math.min(100, Math.max(0, percent)) + '%';
    }
  }
  ...
  INFOSPOTS_CONFIG.forEach(function (cfg) {
    ...
    infospot.position.set(cfg.position[0], cfg.position[1], cfg.position[2]);
